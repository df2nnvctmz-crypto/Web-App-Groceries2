...
const origNameMatch = normalizeText(originalFood.name).includes(q);
const origCatMatch = normalizeText(originalFood.category).includes(q);
const origSubCatMatch = normalizeText(originalFood.subCategory).includes(q);
...
console.log({
  nameMatch, catMatch, subCatMatch, origNameMatch, origCatMatch, origSubCatMatch, synonymMatch, searchMatches
});
